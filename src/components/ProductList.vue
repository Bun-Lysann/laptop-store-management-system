...
import { products, loadProducts, saveProducts, deleteData } from "../backends/dataStorage.js";
...
// Delete a product by serial using centralized storage
const deleteProduct = (indexOrSerial) => {
  // indexOrSerial may be an index (number) or a serial (string)
  if (!confirm("Are you sure you want to delete this product?")) return;

  if (typeof indexOrSerial === "number") {
    // fallback: remove by index
    products.value.splice(indexOrSerial, 1);
    saveProducts();
    return;
  }

  const ok = deleteData(indexOrSerial);
...
const updateProduct = () => {
  ...
  const updated = { ...editProduct.value };
...
<template>
  <div
    class="max-w-7xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-xl font-[Poppins]"
  >
        ...
        >
          <td class="border p-2">
            <img
              ...
              :src="product.image"
              alt="Product Image"
              class="w-8 h-8 rounded-lg object-cover mx-auto"
            />
  ...
  <div
    ...
    class="absolute container mt-40 inset-0 flex items-center justify-center bg-opacity-50"
  >
    <div class="bg-white p-6 rounded-lg shadow-lg w-96">
      ...
      <div class="space-y-3">
        ...
        <div class="flex space-x-2">
          <input
            v-model="editProduct.cpu"
            type="text"
            placeholder="CPU (e.g., Intel i7)"
            class="w-full p-2 border rounded"
          />
          <input
            v-model="editProduct.gpu"
            type="text"
        ...
        <div class="flex space-x-2">
          <input
            v-model="editProduct.display"
            type="text"
            placeholder="Display (e.g., 15.6-inch 1080p)"
            class="w-full p-2 border rounded"
          />
          <input
            v-model="editProduct.battery"
            type="text"
        ...
        <div class="flex space-x-2">
          ...
          />
          <input
            v-model="editProduct.dateRelease"
            type="date"
            class="w-full p-2 border rounded"
          />
        </div>
